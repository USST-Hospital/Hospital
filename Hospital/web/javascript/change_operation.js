...
$(document).ready(function () {
    ...
    $('#adminAddOpeartion').click(function () {

        var operationId = $('#operationId').text();
        ...
        if(isPassPatientName&&year!=-1&&month!=-1&&day!=-1&&startTime!=-1&&endTime!=-1&&nurses.length>0&&operationRoom!=-1&&analgesist!=-1&&operationName!=''&&doctor!='') {
            ...
            $.post("changeOperationSubmit.do",{'info':JSON.stringify(info)},function (data,status) {
...
function appointmentTimeChange() {
    var operationId = $('#operationId').text();
    $.post("changeOperationTimeChange.do",{'operationId':operationId,'date':year+'年'+month+'月'+day+'日','startTime':startTime,'endTime':endTime},function (data,status) {
...
function timeChangeReset() {
    ...
    $('#adminDoctor').html('<option value="-1">未选择</option>');
    $('#nurses').empty();
}
